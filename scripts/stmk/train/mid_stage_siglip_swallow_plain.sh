...
############### mid stage with plain template ################
PROMPT_VERSION=plain

BASE_RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-pretrain_595k_jp_plain"
echo "BASE_RUN_NAME: ${BASE_RUN_NAME}"

MID_RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-mlp2x_gelu-mid_llava_pretrain_ja_plain"
...
torchrun --standalone --nproc_per_node=8 --nnodes=1 \
    ...
    --data_path ./scripts/train/mid_stage_swallow.yaml \
    ...
    --output_dir "./checkpoints/mid_stage/${MID_RUN_NAME}" \
    --num_train_epochs 1 \
